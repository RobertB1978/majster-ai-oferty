...
function createMockSupabaseClient() {
  ...
  const buildQueryBuilder = <T>(mockData: T | T[] | null = null): MockQueryBuilder<T> => {
    ...
    const builder: MockQueryBuilder<T> = {
      ...
      returns: async () => ({ data: normalizeArray(), error: null }),
      then: (resolve, reject) =>
        Promise.resolve<MockQueryResult<T[]>>({ data: normalizeArray(), error: null }).then(resolve, reject),
    };
