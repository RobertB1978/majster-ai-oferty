    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col gap-0 p-0">
        <DialogHeader className="px-6 pt-6 pb-3">
          <DialogTitle>{t('szybkaWycena.bulkAddTitle')}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">{t('szybkaWycena.bulkPasteLabel')}</Label>
              <Button
                variant="outline"
                size="sm"
                type="button"
              ...
              <input
                ...
                aria-label={t('szybkaWycena.bulkLoadCsvAriaLabel')}
              />
            </div>

            <Textarea
              placeholder={t('szybkaWycena.bulkPlaceholder')}
              value={rawText}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between flex-wrap gap-1">
                <Label className="text-sm font-medium">
                  {t('szybkaWycena.bulkPreviewLabel')} ({rows.length}{' '}
                  {rows.length === 1
                    ? t('szybkaWycena.bulkPreviewRow')
                    : t('szybkaWycena.bulkPreviewRows')}
                  )
                </Label>
                {errorCount > 0 && (
                  <span className="flex items-center gap-1 text-xs text-destructive">
                    ...
                    {errorCount}{' '}
                    {errorCount === 1
                      ? t('szybkaWycena.bulkErrorSkipped')
                      : t('szybkaWycena.bulkErrorsSkipped')}
                  </span>
                )}
              </div>
              ...
              <div className="hidden sm:grid grid-cols-[1fr_72px_68px_92px_32px] gap-1.5 px-1 text-xs font-medium text-muted-foreground">
                <span>{t('szybkaWycena.bulkColName')}</span>
                <span>{t('szybkaWycena.bulkColQty')}</span>
                <span>{t('szybkaWycena.bulkColUnit')}</span>
                <span className="text-right">{t('szybkaWycena.bulkColNetPrice')}</span>
                <span />
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <Input
                        ...
                        placeholder={t('szybkaWycena.bulkNamePlaceholder')}
                        aria-label={t('szybkaWycena.bulkNamePlaceholder')}
                      ...
                      <Input
                        ...
                        inputMode="decimal"
                        aria-label={t('szybkaWycena.bulkColQty')}
                      ...
                      <div className="sm:contents grid grid-cols-2 col-span-2 gap-1.5">
                        <Input
                          ...
                          placeholder={t('szybkaWycena.bulkUnitPlaceholder')}
                          aria-label={t('szybkaWycena.bulkColUnit')}
                        />
                        ...
                        <div className="relative">
                          <Input
                            ...
                            inputMode="decimal"
                            aria-label={t('szybkaWycena.bulkColNetPrice')}
                      ...
                      <Button
                        ...
                        aria-label={t('szybkaWycena.bulkDeleteRow')}
        ...
        <DialogFooter className="px-6 py-4 border-t gap-2">
          ...
          >
            {validCount > 0
              ? t('szybkaWycena.bulkConfirmCount', { count: validCount })
              : t('szybkaWycena.bulkConfirm')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
