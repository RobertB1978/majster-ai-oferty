  ...
  const FAQ_ITEMS = [
    {
      ...
      answer: t('landing.faq.a1', 'Nie. Zaczynasz bezpłatnie przez 30 dni, bez zobowiązań i bez karty kredytowej. Możesz anulować w każdej chwili.'),
    },
    {
      id: 'faq-2',
    ...
    },
    {
      ...
      answer: t('landing.faq.a4', 'Plan darmowy jest bezpłatny przez 30 dni. Po tym czasie możesz wybrać płatny plan dopasowany do swoich potrzeb.'),
    },
    {
      id: 'faq-5',
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gray-50 dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-2xl px-6">
          {FAQ_ITEMS.map((item) => {
            ...
            return (
              <div key={item.id} className="border-b border-gray-200 dark:border-[#2A2A2A] last:border-b-0">
                <button
                  id={item.id}
                ...
                >
                  <span className={`font-semibold text-base transition-colors duration-200 ${isOpen ? 'text-amber-500' : 'text-gray-900 dark:text-white hover:text-amber-500'}`}>
                    {item.question}
                  </span>
                  <ChevronDown
                    className={`w-5 h-5 text-gray-400 dark:text-[#A3A3A3] shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
                    aria-hidden="true"
                  />
                </button>
                <div
