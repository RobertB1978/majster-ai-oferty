    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-[#525252] mt-2">Ceny netto · Do każdego zakupu doliczamy 23% VAT</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 max-w-6xl mx-auto">
          {PLANS.map((plan) => (
            <div
              key={plan.slug}
              className={`relative bg-[#1A1A1A] rounded-2xl p-6 flex flex-col gap-5 transition-all duration-300 ${
                plan.highlighted
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[#525252] leading-relaxed">{plan.description}</p>
              </div>
                  ...
                  return (
                    <li key={feature} className="flex items-start gap-2 text-sm">
                      <Check
                        ...
                        aria-hidden="true"
                      />
                      <span className={isPrep ? 'text-[#525252]' : 'text-[#A3A3A3]'}>
                        {feature}
                        {isPrep && (
                          <span className="ml-1.5 text-[10px] font-medium text-[#525252] border border-[#2A2A2A] rounded px-1 py-0.5 uppercase tracking-wide align-middle">
                            W przygotowaniu
                          </span>
                        )}
                      </span>
              ...
              >
                {plan.pricePLN === 0 ? 'Zacznij za darmo' : `Wypróbuj ${plan.name}`}
              </Link>
            </div>
          ))}
        </div>
        ...
        <p className="text-center text-sm text-[#525252] mt-8">
          Anuluj w każdej chwili · Wystawiamy faktury VAT
        </p>
      </div>
    </section>
  );
