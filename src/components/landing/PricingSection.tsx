...
// Features that are beta/in-progress (cross-referenced with features.data.ts)
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 max-w-6xl mx-auto">
          {PLANS.map((plan) => (
            ...
            >
                ...
                {plan.features.map((feature) => {
                  ...
                  return (
                    <li key={feature} className="flex items-start gap-2 text-sm">
                      ...
                      <span className={isPrep ? 'text-gray-400 dark:text-[#525252]' : 'text-gray-600 dark:text-[#A3A3A3]'}>
                        {feature}
                        {isPrep && (
              ...
              <Link
                to={CTA_ROUTE}
                className={`block text-center py-3 px-6 rounded-xl font-semibold text-sm transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 min-h-[44px] flex items-center justify-center ${
                  plan.highlighted
