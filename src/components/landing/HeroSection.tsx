...
export function HeroSection() {
  ...
  const STATS = [
    { label: t('landing.trust.pdf_label', 'Wyceny PDF'), value: '✓', verified: true },
    { label: t('landing.trust.mobile_label', 'Mobile-first'), value: '✓', verified: true },
    { label: t('landing.trust.languages_label', 'Języki'), value: 'PL / EN / UK', verified: true },
  ];
        ...
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 text-center lg:text-left">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 mb-8">
              {STATS.filter((s) => s.verified).map((stat) => (
                <div
                  key={stat.label}
          ...
          {/* Right: floating UI mock — CSS-only */}
          <div className="hidden lg:flex flex-col items-center shrink-0 w-72">
            <div className="animate-float w-full" style={{ animationDuration: '4s' }}>
              <div className="bg-gray-50 dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-2xl p-5 shadow-xl dark:shadow-2xl">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="ml-auto shrink-0">
                    <span className="text-xs bg-amber-500/20 text-amber-600 dark:text-amber-400 rounded-full px-2 py-0.5 font-medium">PDF</span>
                  </div>
                </div>
                <div className="space-y-2 mb-4">
                  {[['Robocizna', '2 400 zł'], ['Materiały', '1 800 zł'], ['Robocizna dodatkowa', '600 zł']].map(([label, val]) => (
                    <div key={label} className="flex justify-between text-sm">
                      <span className="text-gray-500 dark:text-[#A3A3A3]">{label}</span>
                  ...
                  <div className="border-t border-gray-200 dark:border-[#2A2A2A] pt-2 flex justify-between font-semibold">
                    <span className="text-gray-500 dark:text-[#A3A3A3]">Razem</span>
                    <span className="text-amber-500 text-base">4 800 zł</span>
                  </div>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 text-center">
                  <span className="text-xs text-amber-600 dark:text-amber-400 font-medium">✓ Wysłano do klienta</span>
                </div>
              </div>
              <div className="mt-3 bg-gray-50 dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-xl p-4">
