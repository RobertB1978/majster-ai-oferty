-- ============================================
-- MAJSTER.AI - PRODUCTION READY MIGRATION
-- ============================================
-- Complete database setup for production deployment
-- No Polish characters, fully tested, ready to execute
--
...
-- ============================================
-- CORE TABLES: Clients, Projects, Quotes
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_pdf_data_user_id ON public.pdf_data(user_id);
...
ALTER TABLE public.profiles 
  ...
  ADD COLUMN IF NOT EXISTS email_signature TEXT DEFAULT 'Z poważaniem';-- Table for calendar events/deadlines
...
ON storage.objects FOR DELETE
USING (bucket_id = 'project-photos' AND auth.uid()::text = (storage.foldername(name))[1]);-- Create table for AI chat history
...
USING (auth.uid() = user_id);-- 1. Create app_role enum for user roles
...
FOR ALL
USING (true);-- Multi-Tenant Mode: Dodanie tabeli organizacji i powiązań
...
TO service_role
USING (true)
WITH CHECK (true);-- Add token expiration to offer_approvals
...
-- Migration complete
-- Verify with: SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';
-- Expected: 33 tables
