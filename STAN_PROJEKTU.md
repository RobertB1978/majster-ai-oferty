...
|------|----|-------|-----------------|
| 2026-02-21 | SECURITY-BUNDLE-FIX | DONE — Security & Bundle Fix (branch: claude/saas-fix-optimization-UCnUh): A) jsPDF 3×HIGH CVEs RESOLVED (upgrade 4.1.0→4.2.0 via npm audit fix; commit 94a49df); B) exceljs lazy-load: exportUtils chunk 938.90 kB → 2.57 kB (exceljs separate chunk); minimatch 20×HIGH BLOCKED (requires ESLint v10 breaking upgrade; owner action); all gates: build ✅ tsc EXIT 0 ✅ lint EXIT 0 (0 errors) ✅ tests 519/519 ✅ | package-lock.json, src/lib/exportUtils.ts, docs/audit/AUDIT_STATUS.md, docs/audit/AUDIT_LOG.md, STAN_PROJEKTU.md |
